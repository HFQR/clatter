...
use clatter::{remove_ansi_escape_codes, Action, LogEvent, OrderDirection, OrderEvent, PriceEvent};
...
pub fn plot(data: Vec<(i64, f64, f64)>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (_, data1, data2, (max_price, min_price, max_profit, min_profit)) = data_iter.fold(
        (
            0.0,
            vec![(*first_timestamp, *first_price)],
            vec![(*first_timestamp, *first_profit)],
    ...
    chart
        ...
        .light_line_style(RGBColor(41, 45, 48).mix(0.8))
        .bold_line_style(RGBColor(41, 45, 48).mix(0.8))
        .draw()?;
    chart
        ...
        .draw()?;
    chart
        .draw_series(LineSeries::new(data1, &YELLOW.mix(0.9)))?
        .label("mid_price")
        .legend(|(x, y)| PathElement::new(vec![(x, y), (x + 20, y)], YELLOW.mix(0.9)));

    chart
        .draw_secondary_series(LineSeries::new(data2, &GREEN.mix(0.6)))?
        .label("profit")
